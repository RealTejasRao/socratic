...
function renderMarkdownBlock(block: MarkdownBlock, index: number) {
  ...
  if (block.type === "table") {
    return (
      <div key={`table-${index}`} className="mt-6 overflow-x-auto rounded-xl border border-black/12">
        <table className="min-w-full border-collapse text-left">
          ...
          <tbody>
            {block.rows.map((row, rowIndex) => (
              <tr key={`tr-${index}-${rowIndex}`} className="align-top odd:bg-white even:bg-black/1.5">
...
export default async function PrivacyPolicyPage() {
  const privacyPolicyMarkdown = await getPrivacyPolicyMarkdown();
  const blocks = toMarkdownBlocks(privacyPolicyMarkdown);
