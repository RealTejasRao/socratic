...
export default function ColorBends({
  ...
  const materialRef = useRef<
    (THREE.ShaderMaterial & { uniforms: ColorBendsUniforms }) | null
  >(null);
  ...
  useEffect(() => {
    ...
    const uniforms: ColorBendsUniforms = {
      uCanvas: { value: new THREE.Vector2(1, 1) },
      uTime: { value: 0 },
      uSpeed: { value: initialOptions.speed },
      uRot: { value: new THREE.Vector2(1, 0) },
      uColorCount: { value: 0 },
      uColors: { value: uColorsArray },
      uTransparent: { value: initialOptions.transparent ? 1 : 0 },
      uScale: { value: initialOptions.scale },
      uFrequency: { value: initialOptions.frequency },
      uWarpStrength: { value: initialOptions.warpStrength },
      uPointer: { value: new THREE.Vector2(0, 0) },
      uMouseInfluence: { value: initialOptions.mouseInfluence },
      uParallax: { value: initialOptions.parallax },
      uNoise: { value: initialOptions.noise },
    };
    const material = new THREE.ShaderMaterial({
      ...
      transparent: true,
    }) as THREE.ShaderMaterial & { uniforms: ColorBendsUniforms };
    ...
    renderer.setClearColor(0x000000, initialOptions.transparent ? 0 : 1);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < MAX_COLORS; i++) {
      ...
      const nextColor = arr[i];
      if (nextColor) vec.copy(nextColor);
      else vec.set(0, 0, 0);
    }
