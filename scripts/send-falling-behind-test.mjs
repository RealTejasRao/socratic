...
const SUBJECT = "You are falling behind";
const DEFAULT_FROM_NAME = "Socratic";
const RECIPIENT = "ytejas72@gmail.com";
const SITE_URL =
...
function buildMessage({ heroImageUrl, logoUrl }) {
  const toIndex = process.argv.indexOf("--to");
  const recipient =
    toIndex >= 0 ? (process.argv[toIndex + 1] ?? "").trim().toLowerCase() : RECIPIENT;
  const plainTextOnly = process.argv.includes("--plain-text");

  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(recipient)) {
    throw new Error("Usage: node scripts/send-falling-behind-test.mjs --to email@example.com");
  }

  const text = [
    "Hey,",
    "",
    "Real talk: your brain gets duller the longer it goes unused. It's just how thinking works. Skills you don't use just fade away.",
    "",
    "You opened Socratic AI once. Then life happened, and you drifted, like most people do. But the ones who stuck with it, even a few minutes a week, are noticeably sharper at arguing, questioning, and catching bad logic than they were a month ago.",
    "",
    "You already have the account. The only thing missing is you showing up.",
    "",
    "usesocratic.com",
  ].join("\n");
  ...
  if (plainTextOnly) {
    return {
      ...
      text,
      headers: {
        "X-Entity-Ref-ID": `falling-behind-plain-text-${Date.now()}`,
      },
    };
  }
...
`,
    ...
    headers: {
      "X-Entity-Ref-ID": `falling-behind-test-${Date.now()}`,
    },
  };
}
...
async function loadUserEmails() {
  ...
  try {
    const result = await client.query(
      'SELECT "email" FROM "User" WHERE "email" IS NOT NULL ORDER BY "createdAt" ASC',
    );
...
async function main() {
  ...
  const shouldSendAllUsers = process.argv.includes("--all-users");

  if (!shouldSendAllUsers) {
    const result = await sendResendMessage(buildMessage({ heroImageUrl, logoUrl }), apiKey);

    if (!result.ok) {
      throw new Error(`Resend request failed (${result.status}): ${JSON.stringify(result.body)}`);
    }

    const toIndex = process.argv.indexOf("--to");
    const recipient =
      toIndex >= 0 ? (process.argv[toIndex + 1] ?? "").trim().toLowerCase() : RECIPIENT;
    console.log(`Sent falling-behind test to ${recipient}.`);
    console.log(`Resend id: ${result.id ?? "unknown"}`);
    return;
  }

  const recipients = await loadUserEmails();
  const runId = new Date().toISOString().replace(/[:.]/g, "-");
  const logDir = path.resolve(workspaceRoot, "logs");
  const logFile = path.join(logDir, `falling-behind-users-${runId}.jsonl`);
  ...
  for (const [index, email] of recipients.entries()) {
    const message = {
      ...buildMessage({ heroImageUrl, logoUrl }),
      to: [email],
      headers: {
        "X-Entity-Ref-ID": `falling-behind-users-${Date.now()}`,
      },
    };
    const result = await sendResendMessage(message, apiKey);
    const entry = {
